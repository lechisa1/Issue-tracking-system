...
//   try {
//     const { email, full_name, user_type_id, position, role_ids, institute_id, assigned_by, phone_number } = req.body;
...
//         await t.rollback();
//         return res.status(400).json({ message: "Some role IDs are invalid." });
//       }
...
//     // Return user with roles and institute
...
const createUser = async (req, res) => {
  ...
  try {
    const {
      ...
      assigned_by,
      phone_number,
      projects        // array of objects: [{ project_id, main_role, sub_role, role_id, sub_role_id }]
    } = req.body;
    ...
    if (Array.isArray(role_ids) && role_ids.length > 0) {
      ...
      if (roles.length !== role_ids.length) {
        await t.rollback();
        return res.status(400).json({ message: "Some global role IDs are invalid." });
      }
    ...
    // Assign user to projects with main & sub roles
    if (Array.isArray(projects) && projects.length > 0) {
      for (const proj of projects) {
        ...
        if (!projectExists) {
          await t.rollback();
          return res.status(400).json({ message: `Invalid project ID: ${project_id}` });
        }
        // Determine main_role and sub_role from role_id/sub_role_id if provided
        ...
        if (role_id) {
          ...
          if (!role) {
            await t.rollback();
            return res.status(400).json({ message: `Invalid role ID for project: ${role_id}` });
          }
          finalMainRole = role.name;
        }
        if ((finalMainRole === "Developer" || finalMainRole === "QA") && sub_role_id) {
          // Optional: fetch sub_role name from sub_roles table
          const subRole = await SubRole.findByPk(sub_role_id, { transaction: t });
          if (!subRole) {
            await t.rollback();
            return res.status(400).json({ message: `Invalid sub-role ID for project: ${sub_role_id}` });
          }
    ...
    `);
    await t.commit();
    // Return user with roles and projects
    const userWithRelations = await User.findOne({
      where: { user_id: user.user_id },
      include: [
        { model: Role, as: "roles", through: { attributes: ["assigned_at", "is_active"] } },
      ...
      include: [
        {
          model: Role,
          as: "role", 
          attributes: ['role_id', 'name', 'description', 'role_type']
        }
      ]
    });
